  ...
  <div class="selector" :class="{ reduceindex: $store.state.show }">
    <div class="form">
      ...
      <el-select v-model="form.sort" filterable placeholder="请选择排序方式">
        <el-option v-for="item in sort" :key="item.value" :label="item.value" :value="item.sort"></el-option>
      </el-select>
      <div class="title">宠物名称</div>
      <el-input clearable v-model="form.petname" placeholder="请输入宠物名称" @keyup.enter.native="query"></el-input>
      ...
      <el-select v-model="form.age" clearable filterable placeholder="请选择宠物年龄">
        <el-option v-for="item in age" :key="item.age" :label="item.age" :value="item.age"></el-option>
      </el-select>
