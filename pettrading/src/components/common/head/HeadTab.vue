  ...
  <div class="headtab">
    <div class="headmain container">
      ...
      <div
        ...
        :key="index"
        :class="{ active: $route.path === lists[index].path }"
    ...
    <chat
      ...
      :active.sync="active"
      :activeuser.sync="activeuser"
      @send="send"
      @clearnoread="clearnoread"
      v-if="$store.state.showchat == true"
    ></chat>
    <!-- </keep-alive> -->
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    sendpet(form) {
      var msg = {
        type: form.type,
        fromavatar: this.$store.state.avatar.substring(5),
        msg: form.msg,
        touid: form.touid,
        pid: form.pid
      };
